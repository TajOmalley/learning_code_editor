...
const TerminalComponent = forwardRef<TerminalRef, TerminalProps>(({ 
  ...
  const initializeTerminal = useCallback(() => {
    ...
    return terminal;
}, [theme, handleTerminalInput, writePrompt, safeFit]);
  ...
  const searchInTerminal = useCallback((searchText: string) => {
    if (term.current && searchText) {
      // Simple search by highlighting in terminal
      // Note: Without SearchAddon, this is a basic implementation
      term.current.select(0, 0, 0);
    }
  }, []);
  ...
  return (
    <div className={cn("flex flex-col h-full min-h-[200px] bg-background border rounded-lg overflow-hidden", className)}>
      {/* Terminal Header */}
